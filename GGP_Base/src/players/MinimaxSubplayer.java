...
public class MinimaxSubplayer extends Subplayer {
	...
	private int minimax(MachineState state, Role currRole, int playersPlayed) throws MoveDefinitionException, TransitionDefinitionException, GoalDefinitionException {
		if (playersPlayed%super.stateMachine.getRoles().size() == 0 && super.stateMachine.isTerminal(state)) {
			return super.stateMachine.getGoal(state, currRole);
		}
		...
		if (currRole == super.role) {
			...
			for (Move move : super.stateMachine.getLegalMoves(state, currRole)) {
				...
				// use super.stateMachine.getNextStates(state, currRole); to get map<Move, List<MachineState>>
